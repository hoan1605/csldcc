angular.module('newApp').factory('pluginsService', [function () {
    ...
    function handleiCheck() {
        ...
        if (!$().iCheck) return;
        $(':checkbox:not(.js-switch, .switch-input, .switch-iphone, .onoffswitch-checkbox, .ios-checkbox, .neo-check-box), :radio:not(.neo-check-box)').each(function () {
    ...
    function bDatepicker() {
        $('.b-datepicker').each(function () {
            $(this).bootstrapDatepicker({
                ...
                format: $(this).data('template') ? $(this).data('template') : 'dd/mm/yyyy',
            });
    ...
    function formValidation() {
        ...
        if ($('.form-change-password-validation').length && $.fn.validate) {
            /* We add an addition rule to show you. Example : 4 + 8. You can other rules if you want */
            $.validator.methods.operation = function (value, element, param) {
                return value == param;
            };
            $('.form-change-password-validation').each(function () {
                var formValidation = $(this).validate({
                    success: "valid",
                    submitHandler: function () {
                        //alert("Form is valid! We submit it")
                    ...
                    ignore: [],
                    rules: {
                        changePassword2: { equalTo: '#changePassword' },
                    },
                    messages: {
                        changePassword: { required: 'Write your password' },
                        changePassword2: { required: 'Write your password', equalTo: '2 passwords must be the same' },
                    },
                    highlight: function (element, errorClass, validClass) {
                        $(element).closest('.form-control').addClass(errorClass).removeClass(validClass);
                    },
